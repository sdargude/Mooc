...
class VocabEntry(object):
    ...
    def words2charindices(self, sents):
        ...
        return [[[self.start_of_word] + [self.char2id[c] for c in list(word)] + [self.end_of_word] for word in sent] for sent in sents]
        ### END YOUR CODE
    ...
    def to_input_tensor_char(self, sents: List[List[str]], device: torch.device) -> torch.Tensor:
        ...
        ###     previous parts
        #print(sents)
        char_to_ids = self.words2charindices(sents)
        padded_chars = pad_sents_char(char_to_ids, self.char2id['<pad>'])
        sents_tensor = torch.tensor(padded_chars, dtype=torch.long, device=device)
        return sents_tensor.permute(1,0,2).contiguous()
        ### END YOUR CODE
