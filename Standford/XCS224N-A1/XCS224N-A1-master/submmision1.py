...
def distinct_words(corpus):
    ...
    ### SOLUTION BEGIN
    corpus_words = sorted(set([item for sublist in corpus for item in sublist]))
...
def compute_co_occurrence_matrix(corpus, window_size=4):
    ...
    for sentence in corpus:
        ...
        for idx in range(0, size):
            ...
            if right >= size:
                right = size
            row = word2Ind[sentence[idx]]

            # Populate the matrix
            for i in range(left, right):
                col = word2Ind[sentence[i]]
                # Skip the word
                if row == col:
                    continue
...
def reduce_to_k_dim(M, k=2):
    ...
    M_reduced = TruncatedSVD(n_components=2, n_iter=n_iters).fit_transform(M)
